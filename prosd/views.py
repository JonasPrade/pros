...
class ProjectContentShortSchema(ma.SQLAlchemySchema):
    ...
    etcs = auto_field()
    etcs_level = auto_field()
    station_railroad_switches = auto_field()
    ...
    overpass = auto_field()
    buffer_track = auto_field()
    simultaneous_train_entries = auto_field()
    lp_12 = auto_field()
    lp_34 = auto_field()
    bau = auto_field()
...
class BksActionSchemaShort(ma.SQLAlchemyAutoSchema):
    class Meta:
...
class BksActionSchema(ma.SQLAlchemyAutoSchema):
    cluster = ma.Nested(lambda: "BksClusterSchema")
    class Meta:
        model = models.BksAction
        include_fk = True
...
class BksClusterSchema(ma.SQLAlchemyAutoSchema):
    bks_action = ma.Nested(BksActionSchemaShort, many=True)
    class Meta:
