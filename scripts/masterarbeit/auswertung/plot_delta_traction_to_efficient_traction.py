...
def plot_delta_to_efficient_traction(filepath_image_directory, areas, traction):
    ...
    for area in areas:
        ...
        if traction in area.cost_all_tractions.keys():
            ...
            if 1 <= delta_traction < 1.5:
                delta_traction_group = "1 - 1.5"
    ...
    ax.set(title='Delta {traction} zu kosteneffizientester Traktion (relativ)')
    ax.set_axis_off()
    # plt.show()
    plt.savefig(
        filepath,
...
if __name__ == '__main__':
    ...
    filepath_directory_image = '../../../example_data/report_scenarios/s_{scenario_id}/files/'
    traction = 'h2'
