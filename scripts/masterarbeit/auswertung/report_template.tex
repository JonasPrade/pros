...
\captionof{table}{\label{table_\VAR{scenario_id}_kenngrößen_untersuchungsgebiete} Anzahl Untersuchungsgebiete sowie Kilometer nach Traktion Szenario \VAR{scenario_id}}
\begin{center}
    ...
    \BLOCK{ for key in cost_effective_traction }
        \VAR{key | replace("_", " ")} & \num{\VAR{cost_effective_traction[key]['area']}} &  \SI{\VAR{(cost_effective_traction[key]['infra_km'])|round|int}}{\km} & \SI{\VAR{(cost_effective_traction[key]['running_km'])/1000|round|int}}{Tsd. \km}\\
    \BLOCK{ endfor }
	\end{tabularx}
\end{center}
...
\begin{center}
    ...
    \BLOCK{ for key in cost_effective_traction_no_optimised }
        \VAR{key | replace("_", " ")} & \num{\VAR{cost_effective_traction_no_optimised[key]['area']}} &  \SI{\VAR{(cost_effective_traction_no_optimised[key]['infra_km'])|round|int}}{\km} & \SI{\VAR{(cost_effective_traction_no_optimised[key]['running_km'])/1000|round|int}}{Tsd. \km}\\
    \BLOCK{ endfor }
	\end{tabularx}
\end{center}
